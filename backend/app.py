...
async def lifespan(app: FastAPI):
    ...
    uncertainty_engine = UncertaintyEngine(
        predictor.model, predictor.device, mc_passes=settings.MC_DROPOUT_PASSES
    )
...
def _require_runtime_engines() -> tuple[UncertaintyEngine, GradCAMEngine]:
    """Fail clearly if an endpoint is called before FastAPI lifespan startup."""
    if uncertainty_engine is None or gradcam_engine is None:
        raise HTTPException(status_code=503, detail="AI runtime is still starting")
    return uncertainty_engine, gradcam_engine
...
async def diagnose(
    ...
    if not image.filename or not validate_image_extension(image.filename):
    ...
    try:
        ...
        uncertainty = uncertainty_runtime.composite_uncertainty(
        ...
        try:
            ...
            gradcam_runtime.generate(img_path, class_idx, gradcam_path)
        ...
        try:
            ...
            logger.warning(f"ABCD feature extraction failed: {exc}")
            abcd_features = {
                "asymmetry": None,
        ...
        try:
            ...
            generate_report(
                decision, uncertainty, recommendation, patient_data, gradcam_path, report_path
            )
...
def get_history(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    for d in diags:
        try:
            abcd = json.loads(d.abcd_features) if d.abcd_features else None
